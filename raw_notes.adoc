...
* consider one more test doing schema change + slog device?
** I've deployed the cluster for this
** done: check zpool config, add slog device to pool
** done: finish configuring cluster
** done: download backup file to cluster
** in progress: restore backup file to cluster
** TODO: start workload, rerun schema test
** TODO?: start workload, rerun expansion/contraction test

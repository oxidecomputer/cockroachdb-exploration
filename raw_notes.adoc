...
Plan for today (see yesterday TODO):

* get chist working / test it
* characterize client impact during expansion/contraction history based on the client histogram data
...
To recreate the cluster, I'm going to repeat my steps from 2020-09-29, which is basically to deploy a new NVME-based database cluster with three nodes (will keep "loadgen" and "mon" zones from existing clsuter), then start a load generator like this:
